...
Build a **FOSS, GitHub-hosted icon pack** of **simple, instantly recognizable 2D icons** turned into **photo-realistic 3D objects**: slightly extruded, beveled, floating at a gentle angle, studio-lit, with a faint contact shadow. Keep silhouettes faithful to classic icons (e.g. camera, message bubble).
...
- Three-quarter view, ~**15°** tilt, floating, infinite **light-gray** seamless backdrop.  
...
pack:
  ...
  backdrop: "infinite seamless light-gray"
  resolution: 1024
  controlnet:
    type: canny   # or depth
    weight: 0.75
...
icons:
  - id: camera
    ...
    effects:
      tame:
        material: "satin plastic with a soft gloss, subtle studio reflections"
      neon:
        material: "same plastic but with a thin inner emissive glow along the edges and lens rim, faint bloom, still readable"

  - id: messages
    source: catalog/sources/messages.svg
    description: "classic rounded speech-bubble with a small triangular tail, solid blue"
    color: "#2196F3"
    effects:
      tame:
        material: "satin plastic with a soft gloss, subtle studio reflections"
      neon:
        material: "same plastic but with a thin inner emissive glow along the edges, faint bloom, still readable blue"
```
...
The object is a slightly extruded, beveled version of the classic simple 2D icon, keeping the exact silhouette and proportions.
It floats in mid-air at a gentle 15-degree angle, three-quarter view, facing slightly toward the camera.
Studio product photography: single key light from upper-left at 45 degrees, soft fill from the right, subtle rim light, faint contact shadow on an infinite seamless light-gray backdrop.
